...
describe('calculateCurrentStreak', () => {
  ...
  it('ignores duplicate completion dates', () => {
    ...
    expect(calculateCurrentStreak([today, yesterday, today, yesterday], today)).toBe(2);
  });

  it('breaks the streak when a calendar day is missing', () => {
    // Gap between today and twoDaysAgo (yesterday is missing)
    expect(calculateCurrentStreak([today, twoDaysAgo], today)).toBe(1);
    // Gap between yesterday and threeDaysAgo (twoDaysAgo is missing)
